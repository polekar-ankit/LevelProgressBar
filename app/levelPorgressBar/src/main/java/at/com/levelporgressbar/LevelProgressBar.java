package at.com.levelporgressbar;
/**
 ...
 * <p>
 * <h>USE secondMileStone PROPERTY OR setSecondMileStone METHOD  TO SET THE BENCHMARK FOR COLOR CHANGE</h>
 ...
 * USE nLastColor PROPERTY ot setnLastColor METHOD TO SET COLOR TO PROGRESS BAR AFTER IT CROSSING THE MILESTONE
 *
...
public class LevelProgressBar extends ProgressBar {
    ...
    private int nLastColor;
    private int nFirstColor;
    private int nMiddleColor;
    ...
    public void setnLastColor(int nLastColor) {
        this.nLastColor = nLastColor;
    }
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        nLastColor = a.getColor(R.styleable.LevelProgressBar_lastColor, Color.RED);
        nFirstColor = a.getColor(R.styleable.LevelProgressBar_firstColor, Color.GREEN);
        nMiddleColor = a.getColor(R.styleable.LevelProgressBar_middleColor, Color.parseColor("#ff7100"));
        isSingleColor = a.getBoolean(R.styleable.LevelProgressBar_singleColor, false);
        secondMileStone = a.getInt(R.styleable.LevelProgressBar_secondMileStone, getMax());
        firstMileStone = a.getInt(R.styleable.LevelProgressBar_firstMileStone, getMax());
...
//        int lastStop = (int) ((getWidth() * (nSlot * 4)) / 100f);
        ...
        if (!isSingleColor) {
//            color = (progress >= secondMileStone) ? nLastColor : nFirstColor;
            if (progress < firstMileStone)
                color = nFirstColor;
            else if (progress >= firstMileStone && progress < secondMileStone)
                color = nMiddleColor;
            else
                color = nLastColor;
        }
    ...
    public void setSecondMileStone(int secondMileStone) {
        this.secondMileStone = secondMileStone;
    }
